"""Tier 2 UI — Pygame-rendered side panels and dialog overlays.

Replaces Tkinter widgets with custom-drawn panels on a Pygame surface.
Dialogs are rendered as modal overlays with keyboard/mouse interaction.
"""
...
# ---------------------------------------------------------------------------
# Dice log (module-level, mirrors Tkinter version)
# ---------------------------------------------------------------------------
_dice_log: List[str] = []
...
def _mono(size: int) -> pygame.font.Font:
    key = ("mono", size, False)
    if key not in _font_cache:
        _font_cache[key] = pygame.font.SysFont("couriernew,courier,monospace", size)
    return _font_cache[key]
...
class Button:
    """Simple clickable button rendered on a Pygame surface."""
...
class TextInput:
    ...
    def draw(self, surface: pygame.Surface) -> None:
        bg = (55, 65, 81) if self.active else (31, 41, 55)
...
class SidePanel:
    """Renders the right-side info panels (scoreboard, stats, logs, etc.)."""
    ...
    def draw(self, surface: pygame.Surface, game: Any,
             selected_figure: Optional[Any], game_mode: str) -> None:
        """Draw all panels on the right side of the screen."""
        x = PANEL_X
        y = 8
        w = PANEL_WIDTH

        # Background
        pygame.draw.rect(surface, PANEL_BG, (x, 0, w + 10, WINDOW_HEIGHT))
    ...
    def _draw_section(self, surface: pygame.Surface, x: int, y: int,
                      w: int, title: str, lines: List[str],
                      max_height: int = 200) -> int:
        """Draw a labelled section box. Returns y after the section."""
        font_title = _font(FONT_SIZE_LABEL, bold=True)
        font_body = _mono(FONT_SIZE_SMALL)
        line_h = font_body.get_linesize()

        # Title
        title_surf = font_title.render(title, True, TEXT_ACCENT)
        surface.blit(title_surf, (x + PANEL_PADDING, y))
        y += title_surf.get_height() + 2

        # Box
        content_h = min(max_height, max(line_h, len(lines) * line_h + 4))
        box_rect = pygame.Rect(x + PANEL_PADDING, y, w - PANEL_PADDING * 2, content_h)
        pygame.draw.rect(surface, (17, 24, 39), box_rect)
        pygame.draw.rect(surface, PANEL_BORDER, box_rect, 1)
    ...
    def _draw_scoreboard(self, surface: pygame.Surface, x: int, y: int,
                         w: int, game: Any) -> int:
        ...
        return self._draw_section(surface, x, y, w, "Scoreboard", lines, 40)

    def _draw_phase(self, surface: pygame.Surface, x: int, y: int,
                    w: int, game: Any) -> int:
        snapshot = game.snapshot()
        ...
        return self._draw_section(surface, x, y, w, "Phase / Ball", lines, 54)

    def _draw_selected(self, surface: pygame.Surface, x: int, y: int,
                       w: int, game: Any,
                       selected: Optional[Any]) -> int:
        if selected is None:
            return self._draw_section(surface, x, y, w, "Selected Figure",
                                      ["No figure selected"], 20)
        sq = game.board.find_square_of_figure(selected)
        location = "off track"
        if sq is not None:
            location = (f"{game.board.get_sector(sq.sector_index).name} / "
        ...
        if getattr(selected, 'is_towed', False):
            lines.append(f"Towed by: {getattr(selected.towed_by, 'name', '?')}")
        elif getattr(selected, 'towing', []):
            names = ", ".join(getattr(f, 'name', '?') for f in selected.towing)
        ...
        return self._draw_section(surface, x, y, w, "Selected Figure", lines, 110)

    def _draw_penalty_box(self, surface: pygame.Surface, x: int, y: int,
                          w: int, game: Any) -> int:
        lines: List[str] = []
        for team in game.teams:
            ...
            for fig in team.active_figures:
                if fig.is_ready_to_return():
                    continue
                if fig.penalty_time <= 0 and fig.shaken_time <= 0 and fig.rest_time <= 0:
                    continue
        ...
        return self._draw_section(surface, x, y, w, "Penalty / Recovery", lines, 50)

    def _draw_combat(self, surface: pygame.Surface, x: int, y: int,
                     w: int, game: Any) -> int:
        text = "No combat resolved yet"
        result = game.last_phase_result
        if result is not None and result.phase.value == "combat":
            combat_lines = [l for l in result.messages if l]
            if combat_lines:
                text = "\n".join(combat_lines[-MAX_COMBAT_LINES:])
        else:
            recent = [e for e in reversed(game.log)
                      if any(k in e for k in COMBAT_KEYWORDS)]
            if recent:
                text = recent[0]
        lines = text.split("\n")
        return self._draw_section(surface, x, y, w, "Combat Resolution", lines, 54)

    def _draw_dice_log(self, surface: pygame.Surface, x: int, y: int,
                       w: int) -> int:
        lines = _dice_log[-10:] if _dice_log else ["(no rolls yet)"]
        return self._draw_section(surface, x, y, w, "Dice Rolls", lines, 80)

    def _draw_replay_log(self, surface: pygame.Surface, x: int, y: int,
                         w: int, game: Any) -> None:
        font_title = _font(FONT_SIZE_LABEL, bold=True)
        font_body = _mono(FONT_SIZE_SMALL)
        line_h = font_body.get_linesize()
        ...
        box_rect = pygame.Rect(x + PANEL_PADDING, y,
                                w - PANEL_PADDING * 2, remaining)
        pygame.draw.rect(surface, (17, 24, 39), box_rect)
        pygame.draw.rect(surface, PANEL_BORDER, box_rect, 1)
...
class ControlBar:
    """Top control bar with buttons and status labels."""
    ...
    def build(self, callbacks: Dict[str, Callable]) -> None:
        """Create buttons. callbacks maps button name → callable."""
        bx = 8
        bw = 100
        bh = BUTTON_HEIGHT
        by = 8
        self.buttons.clear()
        for name in ["New Match", "Next Phase", "Play Turn", "Gen Teams",
                      "Grab Tow", "Release Tow"]:
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        pygame.draw.rect(surface, (31, 41, 55), bar_rect)
        pygame.draw.line(surface, PANEL_BORDER, (0, 50), (WINDOW_WIDTH, 50))
        ...
        if self._interaction_text:
            int_surf = font.render(self._interaction_text, True, (245, 158, 11))
        ...
        if self._last_action_text:
            ...
            surface.blit(act_surf, (WINDOW_WIDTH - act_surf.get_width() - 16, 28))
...
class DialogBase:
    """Base for modal dialog overlays rendered on top of the game."""
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        pygame.draw.rect(surface, DIALOG_BG, rect)
        pygame.draw.rect(surface, DIALOG_BORDER, rect, 2)
...
class NewMatchDialog(DialogBase):
    ...
    def __init__(self) -> None:
        ...
        self._cvc_btn = Button(
            pygame.Rect(bx, by + 40, 200, 28), "Computer vs Computer",
            lambda: self._set_mode(MODE_CVC))
        self._hvc_btn = Button(
            pygame.Rect(bx + 210, by + 40, 200, 28), "Human vs Computer",
            lambda: self._set_mode(MODE_HVC))

        # Team name inputs
        self._home_input = TextInput(
            pygame.Rect(bx + 150, by + 85, 260, 26), "Home", "Home team name")
        self._vis_input = TextInput(
            pygame.Rect(bx + 150, by + 120, 260, 26), "Visitor", "Visitor team name")

        # Custom team checkboxes (rendered as toggle buttons)
        self._gen_home_btn = Button(
            pygame.Rect(bx, by + 165, 200, 28), "[ ] Gen Home Team",
            self._toggle_gen_home)
        self._gen_vis_btn = Button(
            pygame.Rect(bx + 210, by + 165, 200, 28), "[ ] Gen Visitor Team",
            self._toggle_gen_vis)

        # Start / Cancel
        self._start_btn = Button(
            pygame.Rect(self.x + 120, self.y + 270, 100, 32), "Start Match",
            self._start)
        self._cancel_btn = Button(
            pygame.Rect(self.x + 240, self.y + 270, 100, 32), "Cancel",
            self._cancel)

        self._buttons = [self._cvc_btn, self._hvc_btn,
...
class TeamGenDialog(DialogBase):
    ...
    def __init__(self, side: TeamSide, existing_name: str = "") -> None:
        ...
        self._roll_btn = Button(
            pygame.Rect(self.x + 120, self.y + 440, 100, 30), "Roll Team",
            self._roll_team)
        self._accept_btn = Button(
            pygame.Rect(self.x + 240, self.y + 440, 100, 30), "Accept",
            self._accept)
        self._cancel_btn = Button(
            pygame.Rect(self.x + 360, self.y + 440, 100, 30), "Cancel",
            self._cancel)
    ...
    def _draw_content(self, surface: pygame.Surface) -> None:
        font = _font(FONT_SIZE_TITLE, bold=True)
        title = font.render(
            f"Team Generation — {self.side.value.title()}", True, TEXT_PRIMARY)
        surface.blit(title, (self.x + 20, self.y + 18))
        ...
        # Preview area
        preview_rect = pygame.Rect(self.x + 20, self.y + 90,
                                    self.width - 40, 330)
        pygame.draw.rect(surface, (17, 24, 39), preview_rect)
        pygame.draw.rect(surface, PANEL_BORDER, preview_rect, 1)
...
class CombatTargetDialog(DialogBase):
    ...
    def __init__(self, attacker: Any, opponents: List[Any]) -> None:
        ...
        self._attack_btn = Button(
            pygame.Rect(self.x + 100, self.y + 250, 100, 30), "Attack",
            self._select)
        self._ai_btn = Button(
            pygame.Rect(self.x + 220, self.y + 250, 120, 30), "AI Default",
            self._cancel)
    ...
    def handle_event(self, event: pygame.event.Event) -> None:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            elif event.key == pygame.K_DOWN and self._selected_idx < len(self.opponents) - 1:
                self._selected_idx += 1
            elif event.key == pygame.K_RETURN:
                self._select()
        elif event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            for i in range(len(self.opponents)):
                item_rect = pygame.Rect(self.x + 20, list_y + i * lh,
                                        self.width - 40, lh)
    ...
    def _draw_content(self, surface: pygame.Surface) -> None:
        font = _font(FONT_SIZE_TITLE, bold=True)
        title = font.render(f"{self.attacker.name} can attack:", True, TEXT_PRIMARY)
        surface.blit(title, (self.x + 20, self.y + 18))
        ...
        for i, opp in enumerate(self.opponents):
            ...
            text = f"{opp.name}  COM {opp.combat}  TGH {opp.toughness}  [{status}]"
            bg_color = (55, 65, 81) if i == self._selected_idx else None
            if bg_color:
                pygame.draw.rect(surface, bg_color,
                                 (self.x + 20, list_y + i * lh,
                                  self.width - 40, lh))
...
class EscalateDialog(DialogBase):
    ...
    def __init__(self, figure: Any, opponent: Any) -> None:
        ...
        self._yes_btn = Button(
            pygame.Rect(self.x + 60, self.y + 140, 140, 30), "Man-to-Man",
            self._yes)
        self._no_btn = Button(
            pygame.Rect(self.x + 220, self.y + 140, 120, 30), "Disengage",
            self._no)
    ...
    def _draw_content(self, surface: pygame.Surface) -> None:
        ...
        prompt = font_sm.render("Escalate to man-to-man combat?", True, TEXT_ACCENT)
...
class TowBarDialog(DialogBase):
    ...
    def __init__(self, biker: Any, candidates: List[Any]) -> None:
        ...
        self._attach_btn = Button(
            pygame.Rect(self.x + 100, self.y + 270, 130, 30), "Attach Selected",
            self._select)
        self._none_btn = Button(
            pygame.Rect(self.x + 250, self.y + 270, 130, 30), "None / AI",
            self._none)
    ...
    def _draw_content(self, surface: pygame.Surface) -> None:
        font = _font(FONT_SIZE_TITLE, bold=True)
        title = font.render(
            f"{self.biker.name} — Tow Bar (max {self._max} more)", True, TEXT_PRIMARY)
        surface.blit(title, (self.x + 20, self.y + 18))
...
class ScoringDialog(DialogBase):
    ...
    def __init__(self, shooter: Any, modifiers: List[Tuple[str, int]]) -> None:
        ...
        self._shoot_btn = Button(
            pygame.Rect(self.x + 60, self.y + 230, 120, 30), "Shoot!",
            self._shoot)
        self._hold_btn = Button(
            pygame.Rect(self.x + 200, self.y + 230, 140, 30), "Hold — circle",
            self._hold)
    ...
    def _draw_content(self, surface: pygame.Surface) -> None:
        font = _font(FONT_SIZE_TITLE, bold=True)
        title = font.render(
            f"{self._shooter.name} in the goal square!", True, TEXT_PRIMARY)
        surface.blit(title, (self.x + 20, self.y + 18))
        ...
        for name, val in self._modifiers:
            sign = "+" if val >= 0 else ""
            txt = mono.render(f"  {name}: {sign}{val}", True, TEXT_SECONDARY)
...
class PackFormationDialog(DialogBase):
    ...
    def __init__(self, packs: List[List[Any]]) -> None:
        ...
        self._form_btn = Button(
            pygame.Rect(self.x + 100, self.y + 250, 130, 30), "Form Selected",
            self._select)
        self._none_btn = Button(
            pygame.Rect(self.x + 250, self.y + 250, 100, 30), "No Packs",
            self._none)
...
class GameOverDialog(DialogBase):
    ...
    def __init__(self, result: str, scores: Dict[str, int]) -> None:
        ...
        self._scores = scores
        self._ok_btn = Button(
            pygame.Rect(self.x + 150, self.y + 140, 100, 30), "OK",
            self._close)
...
class MessageDialog(DialogBase):
    ...
    def __init__(self, title_text: str, message: str) -> None:
        ...
        self._ok_btn = Button(
            pygame.Rect(self.x + 150, self.y + 130, 100, 30), "OK",
            self._close)
        self._buttons = [self._ok_btn]
